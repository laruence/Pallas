...
<title><?php echo $lang['news_edit']?></title>
<style type="text/css">
<!--
.STYLE2 {color: #FF0000}
-->
...
<?php  
	...
	{ 
		//echo "<P>".$news_text."</p>";
		if($id!=$artid){ echo "<script>alert('参数非法！');history.go(-1);</script>"; exit(-1);} //验证url和form中的id一致 

		//$news_text=preg_replace("/(\r\n|\n)/","<br>",$news_text);
		if($defaultpicurl=="nopic.gif") $picnews=0;
		else $picnews=1;
		if($cmt==NULL) $cmt=0;

		$news_text = urldecode($news_text);
		$ip=$_SERVER['REMOTE_ADDR'];
		$sql="update $T_NEWS set news_title='$news_title',news_cat='$catid',news_spec='$specid',news_author='$author',news_from='$copyfromname',news_text='$news_text',news_picurl='$defaultpicurl',news_cmt='$cmt',news_picnews=$picnews,news_placard=$placard,news_ip='$ip',news_type='$newstype',news_pictrues='$pictrues',news_abstract='$abstract' ,news_typein='$typein',news_type1='$type1',news_type2='$type2', news_keywords='$keywords' where news_id=$artid";

		$result = mysql_query($sql) or die (mysql_error());
		$d_file = ($glo_web_path?$glo_web_path:"")."category/".$catid.($specid?"/".$specid:"")."/".$artid.".html";
		@unlink($d_file);
		$action="修改新闻: ".$news_title;
		wLog($_SESSION['loginname'],$date,$action,$ip);


		echo "<script>alert('修改成功!');history.go(-1);</script>";
	}
	$sql="select * from $T_NEWS where news_id = $id";
	$result = mysql_query ($sql) or die (mysql_error());
	$rows = mysql_fetch_array($result,MYSQL_BOTH);
	$result_cat=$rows['news_cat'];
	$result_spec=$rows['news_spec'];
	if($result_spec==0)
	{
		$sql="select * from $T_CAT where cat_id=$result_cat";
		$result=mysql_query($sql);
		$result_num=mysql_num_rows($result);
		if($result_num==0) {$result_cat=0; $result_spec=0;}
	}
	else
	{

		$sql="select * from $T_CAT where cat_id=$result_spec and cat_parent=$result_cat";
		$result=mysql_query($sql);
		$result_num=mysql_num_rows($result);
		if($result_num==0) {$result_cat=0; $result_spec=0;}
	}
	...
	$rows['news_text']=addslashes($rows['news_text']);
	$result_content=preg_replace("/(\r\n|\n)/"," ",$rows['news_text'])
?>
...
function changecmt()
{
	if(document.myform.cmt.value==0)
	{

		document.myform.cmt.value=1;

	}
	else {
			document.myform.cmt.value=0;
	}
} 

function setvar(){

	var catid="<?php echo $result_cat?>";
	var specid="<?php echo $result_spec?>";
	loadcatid(catid);//获取栏目
	setTimeout(function(){loadspecid(catid, specid);}, 100);
	if(<?php echo $rows['news_cmt']?>)document.myform.cmt.checked=true;

	document.myform.news_title.value="<?php echo $rows['news_title']?>";
	document.myform.copyfromname.value="<?php echo $rows['news_from']?>";
	document.myform.defaultpicurl.value="<?php echo $rows['news_picurl']?>";
	document.myform.newstype.value="<?php echo $rows['news_type']?>";
	document.myform.placard.value="<?php echo $rows['news_placard']?>";
	document.myform.author.value="<?php echo $rows['news_author']?>";
	document.myform.typein.value="<?php echo $rows['news_typein']?>";
	var pic_list="<?php echo $rows['news_pictrues']?>";//新闻图片
	pic_list=pic_list.split("|");
	for(i=1;i<pic_list.length;i++){ 
		filename=pic_list[i].split(",");
		if(filename[0]=="<?php echo $rows['news_picurl']?>")
		{
			myform.news_pic.options[myform.news_pic.length]=new Option(filename[1],filename[0],true,true);
		}
		else
		{
			myform.news_pic.options[myform.news_pic.length]=new Option(filename[1],filename[0]);
		}

	}
		/*
			if(message.contentWindow.document.body != undefined)
			message.contentWindow.document.body.innerHTML = "<?php echo $result_content?>";
		else
...
<table width="100%" border="0" align=center cellpadding="2" cellspacing="1" class="tableBorder" >
  <tr>
	<td class="submenu" align=center><?php echo $lang['news_edit']?></td>
  </tr>
  <tr>
...
<table  width="100%" align="center" border="0" cellspacing="0" cellpadding="0">
  <form action="<?php $PHP_SELF ?>?action=commit&id=<?php echo $rows['news_id']?>" method="post" name="myform" onSubmit="return doCheck();">
	<tr>
	 <input type="hidden" name=artid id=artid value=<?php echo $rows['news_id'];?>>
	  <td width="8%" class="tablerow STYLE2"><?php echo $lang['n_sort']?></td>
	  <td class="tablerow" width="92%"><select onChange=" loadspecid(this.value); " id='catid' name='catid'>
		<option value='0'><?php echo $lang['n_sort_s']?></option>
	  </select>
	  <font color='#FF0000'>* </font>      </td>
	</tr>
	<tr>
	  <td class="tablerow" width="8%"><?php echo $lang['n_spec']?></td>
	  <td  id="0" name="0" class="tablerow"  width="92%" style="display:inline "><select name='specid' id='specid'  disabled>
	...
	<tr>
	  ...
	  <td width="92%" class="tablerow"><table width='100%' border='0' cellpadding='0' cellspacing='0'>
		<tr>
		  <td class="tablerow"><input type="hidden" name="orititle" id="orititle" >
			  ...
			  <select name='titlefonttype' id='titlefonttype' onChange="changetitlestyle(this.value);">
				...
				<option value='0'><?php echo $lang['n_rule']?></option>
			  </select></td>
		</tr>
	  </table></td>
	</tr>
	<tr>
	  ...
	  <td class="tablerow"><?php echo $lang['n_attr_1']?>: <input type='text' name="type1" value="<?php echo $rows['news_type1']?>" >
	  &nbsp;<?php echo $lang['n_attr_2']?>:<input name="type2" type="text" id="type2" value="<?php echo $rows['news_type2']?>"></td>
	</tr>
	<tr>
	  <td class="tablerow STYLE2"><?php echo $lang['n_abstract']?></td>
	  <td class="tablerow"><textarea name="abstract" cols="70" rows="3" wrap="virtual"   style="overflow:hidden;" ><?php echo $rows['news_abstract']?>
</textarea>      </td>
	</tr>
	<tr>
	  <td class="tablerow " width="8%"> <?php echo $lang['n_keywords']?></td>
	  <td class="tablerow " width="92%"><input name="keywords" type="text"   style="overflow:hidden;"  size="70" maxlength="70" value="<?php echo $rows['news_keywords']?>" /></td>
	</tr>
	<tbody style="display:''" id="article">
	  <tr>
		...
		<td class="tablerow"><input name='author' type='text' id='author'  size='30' maxlength='30' >
			&nbsp; &nbsp;&nbsp;<?php echo $lang['n_typein']?> 
			 <input type="text" name="typein" id="typein"  size=20 maxlength="20"><font color='blue'><=[<a href='#' onClick="document.myform.typein.value='<?php echo $lang['n_unknown']?>'"><font color='green'><?php echo $lang['n_unknown']?></font></a>] [<a href='#' onClick="document.myform.typein.value='<?php echo $lang['n_anonymous']?>'"><font color='green'><?php echo $lang['n_anonymous']?></font></a>] [<a href='#' onClick="document.myform.typein.value='<?php echo $loginname?>'"><font color='green'><?php echo $loginname?></font></a>]</font></td>
	  </tr>
	  <tr>
		...
		<td class="tablerow"><table width='100%' border='0' cellpadding='0' cellspacing='0'>
		  <tr>
			<td width='29%'><?php echo $lang['n_from_1']?>
			  ...
			  <font color='blue'><=[<a href='#' onClick="document.myform.copyfromname.value='<?php echo $lang['n_original']?>'"><font color='green'><?php echo $lang['n_original']?></font></a>]</font> </td>
	...
	function checkdata(theform)
	{
		...
		if (theform.viewhtml.checked == true)
		{
			...
			<select name="selectFont" onChange="FormatText('fontname', selectFont.options[selectFont.selectedIndex].value);document.myform.selectFont.options[0].selected = true;"  style="font-family: 宋体; font-size: 9pt" onmouseover="window.status='选择选定文字的字体。';return true;" onmouseout="window.status='';return true;">
			  <option selected><?php echo $lang['n_select_font']?></option>
			  <option value="宋体"><?php echo $lang['n_s1']?></option>
			  <option value="楷体_GB2312"><?php echo $lang['n_s2']?></option>
			  <option value="新宋体"><?php echo $lang['n_s3']?></option>
			  <option value="黑体"><?php echo $lang['n_s4']?></option>
			  <option value="隶书"><?php echo $lang['n_s5']?></option>
			  <option value="幼圆"><?php echo $lang['n_s6']?></option>
			...
			<select name="selectColour" onChange="FormatText('ForeColor',selectColour.options[selectColour.selectedIndex].value);document.myform.selectColour.options[0].selected = true;" onmouseover="window.status='选择选定文字的颜色。';return true;" onmouseout="window.status='';return true;">
			  <option value="0" selected><?php echo $lang['n_font_color']?></option>
			...
			<select name="selectbgColour" onChange="FormatText('BackColor',selectbgColour.options[selectbgColour.selectedIndex].value);document.myform.selectbgColour.options[0].selected = true;" onmouseover="window.status='选择选定文字的背景颜色。';return true;" onmouseout="window.status='';return true;">
			  <option value="0" selected><?php echo $lang['n_bg_color']?></option>
			  <option style="background-color:#F0F8FF;color: #F0F8FF" value="#F0F8FF">#F0F8FF</option>
			  <option style="background-color:#FAEBD7;color: #FAEBD7" value="#FAEBD7">#FAEBD7</option>
			  <option style="background-color:#00FFFF;color: #00FFFF" value="#00FFFF">#00FFFF</option>
			  <option style="background-color:#7FFFD4;color: #7FFFD4" value="#7FFFD4">#7FFFD4</option>
			...
			<select language="javascript"  id="FontSize" title="字号大小" onChange="FormatText('fontsize',this[this.selectedIndex].value);" name="select" onmouseover="window.status='选择选定文字的字号大小。';return true;" onmouseout="window.status='';return true;">
			  <option class="heading" selected><?php echo $lang['n_font_size']?> </option>
			  <option value="7"><?php echo $lang['n_font_11']?> </option>
			  <option value="6"><?php echo $lang['n_font_12']?> </option>
			  <option value="5"><?php echo $lang['n_font_13']?> </option>
			  <option value="4"><?php echo $lang['n_font_14']?> </option>
			  <option value="3"><?php echo $lang['n_font_15']?> </option>
			  <option value="2"><?php echo $lang['n_font_16']?> </option>
			  <option value="1"><?php echo $lang['n_font_17']?></option>
			</select>
		  ...
		  <img src=./editor/image/s.gif /><br />
	<iframe id="message" width="550" height="275" src="./editor/textarea.htm"></iframe>
		</td>
	  </tr>
	  ...
	  <tr>
		...
		<td class="tablerow"> 
		<input  type="hidden" name="pictrues" id="pictrues" value="<?php echo $rows['news_pictrues']?>">
	  ...
	  <tr>
		<td class="tablerow"><?php echo $lang['n_type2']?></td>
		<td class="tablerow">
		<?php echo $lang['n_cmt_admit']?>
		<input name="cmt" type="checkbox"  value="1" >
		&nbsp;<?php echo $lang['n_class']?> &nbsp;
	...
	<tr>
	  ...
	  <td class="tablerow"><input type="submit" name="Submit" value="<?php echo $lang['submit']?>">
		&nbsp;
		<input type="reset" name="Reset" value="<?php echo $lang['clear']?>">      </td>
	</tr>
  </form>
</table>
</body>
